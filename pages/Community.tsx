  ...
  return (
    <div className="bg-gray-50 min-h-full">
       ...
       <div className="flex overflow-x-auto space-x-2 p-4 bg-white border-b border-gray-100 no-scrollbar">
          {['推荐', '综合', '攻略', '同人', '问答'].map((topic, idx) => (
             <span key={topic} className={`px-4 py-1.5 rounded-full text-sm font-medium whitespace-nowrap ${idx === 0 ? 'bg-gray-900 text-white' : 'bg-gray-100 text-gray-600'}`}>
               {topic}
             </span>
          ))}
       </div>
