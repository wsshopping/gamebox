        ...
        <div className="relative h-52 rounded-[32px] bg-gradient-to-br from-slate-900 via-[#020617] to-slate-900 p-8 overflow-hidden shadow-2xl shadow-black/30 border border-white/10 group">
           ...
           <div className="relative z-10 flex flex-col h-full justify-between text-slate-800">
              <div className="flex justify-between items-start">
                 <div>
                    <h1 className="text-2xl font-black tracking-tight italic drop-shadow-md text-white">PLATINUM <span className="text-amber-500">VIP</span></h1>
              ...
              <div className="flex items-end justify-between">
                 <div>
                    ...
                    <p className="text-4xl font-mono font-bold text-amber-500 tracking-tighter drop-shadow-[0_0_10px_rgba(245,158,11,0.5)]">1,250</p>
                 </div>
      ...
      <div className="-mt-10 px-6 pb-24 relative z-10">
                ...
                <div className="h-40 flex items-center justify-center relative mb-4">
                  {reward ? (
                    <div className="animate-fade-in-up">
                       ...
                       <p className="text-amber-500 font-bold text-lg">{reward}</p>
        ...
        <div className="card-bg rounded-[24px] border border-theme p-6 mb-6">
          <div className="flex justify-between items-center mb-6">
             ...
             <span className="text-xs font-bold text-accent bg-[var(--bg-primary)] border border-theme px-2 py-1 rounded-md">赢取好礼</span>
          </div>
              ...
              return (
                <div key={day} className={`flex flex-col items-center group cursor-pointer`}>
                  <div className={`w-9 h-9 rounded-full flex items-center justify-center text-sm mb-2 transition-all border-2 ${
                     ...
                     : 'bg-transparent border-slate-300 text-slate-400 group-hover:border-slate-400'
                  }`}>
                     {active ? '✓' : ''}
                  </div>
                  <span className={`text-[10px] font-bold ${active ? 'text-accent' : 'text-slate-400'}`}>{day}</span>
                </div>
              );
            })}
          </div>
        ...
        <div className="space-y-4">
           ...
           {isLoading ? (
             ...
             tasks.map((task, index) => (
              <div key={task.id} className="card-bg p-5 rounded-[20px] shadow-sm border border-theme flex items-center justify-between hover:bg-[var(--bg-primary)] transition-all">
                ...
                <button 
                  ...
                  className={`text-xs px-5 py-2.5 rounded-full font-bold transition-all ${
                    task.status === 'completed' 
                      ...
                      : 'bg-transparent border-2 border-slate-400 text-slate-400 hover:border-slate-500 hover:text-slate-500'
                  }`}
                >
