...
const GroupDetail: React.FC = () => {
  ...
  // Status: 'unknown', 'joined', 'not_joined', 'pending'
  const [relationStatus, setRelationStatus] = useState<'unknown' | 'joined' | 'not_joined'>('unknown');
  ...
  useEffect(() => {
    const loadGroup = async () => {
      ...
      setIsLoading(true);
      try {
        // UPDATED: Use api.group.getDetail
        const data = await api.group.getDetail(id);
        setGroup(data);
        // In a real app, 'data' should contain a field like `isMember` or `relation`
        // For now, we simulate this logic: 
        // If it's a "joined" group, set joined. Otherwise check mock logic or API response.
        if (data) {
             // Mock logic: assume we are joined if it's one of the main chat list groups (g1, g2)
             // Real logic: setRelationStatus(data.isMember ? 'joined' : 'not_joined');
             setRelationStatus(['g1', 'g2'].includes(data.id) ? 'joined' : 'not_joined');
        }
      } catch (err) {
  ...
  const handleJoin = async () => {
    ...
    try {
      // UPDATED: Use api.group.join
      const success = await api.group.join(id, 'Applying to join'); 
      if (success) {
          // Optimistically update UI
          setRelationStatus('joined');
          // Optional: Show a toast here
      }
    } catch (e: any) {
      console.error(e);
      alert(`加入失败: ${e.message || '网络错误'}`);
    } finally {
  ...
  if (!group) {
    return (
      <div className="app-bg min-h-screen flex flex-col items-center justify-center">
         <p className="text-slate-500">群组不存在或网络错误</p>
         <button onClick={() => navigate(-1)} className="mt-4 text-indigo-600 font-bold">返回</button>
      </div>
    );
  }
          ...
          <div className="max-w-md mx-auto">
             {relationStatus === 'joined' ? (
                <button 
                  ...
                  className="w-full bg-[var(--text-primary)] text-[var(--bg-primary)] font-bold py-3.5 rounded-xl shadow-lg hover:opacity-90 transition-opacity"
                >
                  进入聊天
                </button>
             ) : (
                <button 
                  ...
                  className={`w-full bg-gradient-to-r from-indigo-600 to-violet-600 text-white font-bold py-3.5 rounded-xl shadow-lg shadow-indigo-500/20 active:scale-95 transition-all ${isSubmitting ? 'opacity-70 cursor-not-allowed' : ''}`}
                >
                  {isSubmitting ? '正在加入...' : '加入群聊'}
                </button>
             )}
          </div>
       </div>
